(() => {
  ...
  // HERO — premium product-launch entrance and depth.
  ...
  // PHYSICAL SYSTEM — pinned desktop story. The pinned element itself is not animated;
  // only its nested visual/copy layers move, matching ScrollTrigger's recommended pattern.
  ...
  if (trolleyStory) {
    ...
    mm.add('(min-width: 851px)', () => {
      ...
      tl.to('.trolley-frame',{scale:.9,rotation:-1,xPercent:-4,duration:1,ease:'none'})
        ...
        .to(items[0],{opacity:.35,x:0,duration:.15,ease:'none'})
  ...
  // DIGITAL SIGNAL — one visual packet travels through the workflow.
  ...
  // LIVE DATA — evidence comes forward, then the metrics resolve.
  ...
  // Chapter indicator + progress dots. Each scene owns one dot.
  ...
  // Gentle ambient motion on system rings.
